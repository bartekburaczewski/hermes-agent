...
RUN apt-get -o Acquire::Retries=3 update && \
    apt-get -o Acquire::Retries=3 install -y --no-install-recommends \
    ca-certificates curl iputils-ping python3 python-is-python3 ripgrep ffmpeg gcc g++ make cmake python3-dev python3-venv libffi-dev libolm-dev libatomic1 procps git openssh-client docker-cli xz-utils sudo && \
    rm -rf /var/lib/apt/lists/*
...
# Passwordless sudo for the hermes user. Sudoers rule is keyed on the
# username, which stage2-hook.sh's `usermod -u` preserves across
# HERMES_UID remaps, so this survives runtime UID overrides. No TTY is
# ever available (non-interactive agent process), so password auth
# would just be an unusable dead end - NOPASSWD is the only viable mode.
